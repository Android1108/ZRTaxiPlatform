...
public class CivilizedSocietyAdapter extends BaseListAdapter<CivilizationArticleBean> {
    public CivilizedSocietyAdapter(List<CivilizationArticleBean> list) {
        super(list);
    }

    @Override
    public void getItemView(int position, View convertView, ViewGroup parent, int type) {
        ArticleAdapter.ViewHolder viewHolder = (ArticleAdapter.ViewHolder) convertView.getTag();
        if (viewHolder == null) {
